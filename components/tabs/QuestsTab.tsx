...
export function QuestsTab({ s, update, updScen, sc, campaignId }: { s:CampaignState; update:U; updScen:(fn:(sc:any)=>any)=>void; sc:any; campaignId:string|null }) {
  ...
  // Apertura locale: la tab si presenta con tutti i box chiusi, così il primo
  // colpo d'occhio è l'elenco degli scenari e non il contenuto di uno solo.
  // L'oro continua a segnalare quale sia lo scenario in gioco.
  const [openScen, setOpenScen] = useState<string>('');
  ...
  return (
    <div>
      ...
      {/* Quest attive — raccolta trasversale agli scenari, sotto il riquadro
          principale. Parte chiusa e l'apertura è locale, secondo la regola per
          cui nessun toggle di navigazione finisce nello stato condiviso. */}
      {longTerm.length > 0 && (
