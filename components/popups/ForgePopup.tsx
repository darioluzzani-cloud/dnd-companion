...
// ─── POPUP: FUCINA DI DURNA — potenziamenti da fabbro ────────
export function ForgePopup({ s, update, campaignId, onClose }: { s: CampaignState; update: U; campaignId: string | null; onClose: () => void }) {
  ...
  const ready = !!(player && item && upgrade && materialOk && !alreadyApplied);
    ...
    update(prev => ({
      players: prev.players.map(pl => {
        ...
        let inventory = pl.inventory.map(it =>
          it.id === item.id
            ? ({ ...it, upgrades: [ ...((it as any).upgrades || []), { name: upgrade.name, desc: upgrade.desc } ] } as any)
            : it
        );
        if (upgrade.material) {
          inventory = inventory
  ...
  return (
    <div className="alchemy-overlay" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="alchemy-popup sheet-popup" style={{ position: 'relative', overflow: 'hidden', borderColor: 'var(--ember)' }}>
        ...
        <div style={{ position: 'relative', zIndex: 2 }}>
          ...
          <div className="card">
            ...
            {item && ((item as any).upgrades || []).length > 0 && (
              <div className="small muted" style={{ marginTop: 6 }}>Già potenziato: {((item as any).upgrades || []).map((u: any) => u.name).join(', ')}</div>
            )}
          </div>
          ...
          <button className="btn btn-primary" disabled={!ready}
            ...
            onClick={forge}>
            {alreadyApplied ? 'Potenziamento già applicato' : 'Creazione'}
          </button>
          {done && (
